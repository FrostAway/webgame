...
//                echo $data['custom_bg'];
    ...
    foreach ($tpcrn_typo_options as $name => $value) {
        ...
        if (!empty($value['color']) || !empty($value['face']) || !empty($value['style']) || !empty($value['size'])) {
            ...
            if (!empty($value['face']))
                echo "font-family: " . $value['face'] . ";";
            if (!empty($value['style']))
                echo "font-weight: " . $value['style'] . ";";
            if (!empty($value['size']))
                echo "font-size: " . $value['size'] . ";";
            echo '}';
            echo "\n";
        }
    } /* for* */
    ...
    <?php if (!empty($data['custom_bg_upload']) && $data['custom_full_bg_img'] == 'yes') { ?>
    ...
    if ($user_ID) :
        ...
        <div id="themepacific-login-form">
            <form action="<?php echo home_url() ?>/wp-login.php" method="post">
                <p id="tpcrn-username"><input size="40" type="text" name="my_username" value="<?php _e('Username', 'bresponZive') ?>" onfocus="if (this.value == '<?php _e('Username', 'bresponZive') ?>') {
                                    this.value = '';
                                }" onblur="if (this.value == '') {
                                            this.value = '<?php _e('Username', 'bresponZive') ?>';
                                        }"   /></p>
                <p id="tpcrn-pw"><input size="40" type="password" name="my_password" value="<?php _e('Password', 'bresponZive') ?>" onfocus="if (this.value == '<?php _e('Password', 'bresponZive') ?>') {
                                    this.value = '';
                                }" onblur="if (this.value == '') {
                                            this.value = '<?php _e('Password', 'bresponZive') ?>';
                                        }"  /></p>
...
function bresponZive_themepacific_breadcrumb() {
    if (!is_home()) {
        ...
        if (is_category() || is_single() || is_tax('fl_guide_cat')) {
            ...
            if (is_category()) {
                ...
                if (is_tax('fl_guide_cat')) {
                    $terms = get_the_terms(get_the_ID(), 'fl_guide_cat');
                    if(is_array($terms)){
                        foreach ($terms as $term){
                             $term_link = get_term_link($term, 'fl_guide_cat');
                             echo '<li><a class="vca" href="'.esc_url($term_link).'">'.$term->name.' &raquo;</a></li>';
                             break;
                        }
                    }
                }
            ...
            <li id="comment-<?php comment_ID(); ?>">
                <div <?php comment_class('comment-wrapper'); ?> >
                    ...
                    <div class="comment-meta">
                        <?php
                        printf(__('%1$s  %2$s  ', 'bresponZive'), sprintf('<div class="author">%s</div>', get_comment_author_link()), sprintf('%4$s<a href="%1$s"><span class="time" style="border:none;">%3$s</span></a>', esc_url(get_comment_link($comment->comment_ID)), get_comment_time('c'), get_comment_date(), sprintf(__('<span class="time">%1$s </span>', 'bresponZive'), get_comment_time())
                                )
                        );
                        ?>
