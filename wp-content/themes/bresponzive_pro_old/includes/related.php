 <?php
 	  global $data;
	           if (strip_tags($data['re_post_no']) > 9){
				...
				?>
				<div class="tpcrn_related_post clearfix" > <h3 class="tpcrn_r_p"><?php _e('Bài liên quan', 'iz_theme'); ?></h3><ul class="tpcrn_r_p_blocks">
		...
		if($args){$tpcrn_r_p_loop = new WP_Query( $args );
                ...
                if ($tpcrn_r_p_loop->have_posts()) { 
                  ...
                  while ($tpcrn_r_p_loop->have_posts()) : $tpcrn_r_p_loop->the_post();?>
                    ...
                    <p><?php _e('Không có bài nào.', 'iz_theme'); ?></p>
                <?php } ?></div> 